...
function MenuItemCard({ item, onNotice }: { item: SellerMenuItem; onNotice: (message: string) => void }): React.JSX.Element {
  ...
  return (
    <Card className="grid gap-4 border-[#eadfce] p-4 shadow-none sm:grid-cols-[100px_minmax(0,1fr)] lg:grid-cols-[100px_minmax(0,1fr)_auto] lg:items-center">
      <img src={item.imageUrl} alt="" className="h-[100px] w-[100px] rounded-xl object-cover" />
      ...
      <div className="col-span-full flex flex-wrap items-center justify-end gap-3 lg:col-span-1 lg:flex-nowrap">
        ...
        <Button variant="secondary" size="sm" className="ml-2 rounded-lg bg-[#fff1d6] text-ilarica-muted hover:bg-[#f8e4bd]" onClick={() => onNotice(`Edição de ${item.name} será conectada à API posteriormente.`)}>Editar</Button>
        <Button variant="danger" size="sm" className="rounded-lg bg-[#ffe7e7] text-[#e22626] hover:bg-[#ffd5d5]" onClick={() => remove(item.id)}>Excluir</Button>
      </div>
    </Card>
  );
}
...
function BusinessHoursPanel({ onSaved }: { onSaved: () => void }): React.JSX.Element {
  ...
  return (
    <Card className="mt-8 border-0 p-6 shadow-none sm:p-8">
      <div className="flex flex-wrap items-center justify-between gap-4"><h2 className="font-display text-xl font-extrabold text-[#7a1e1e]">Horário de Funcionamento</h2><Button variant="ghost" className="h-9 rounded-xl border border-ilarica-orange px-4 text-sm text-ilarica-orange hover:bg-[#fff0e8]" onClick={onSaved}>Salvar Alterações</Button></div>
    ...
    <main className="min-h-screen bg-[#fff1d6] text-ilarica-ink">
      ...
      <div className="mx-auto grid w-full max-w-[1440px] gap-6 px-5 py-7 sm:px-8 lg:grid-cols-[280px_minmax(0,1fr)] lg:gap-10 lg:px-16 lg:py-10">
        ...
        <section>
          ...
          <div className="mt-7 space-y-4">{items.map((item) => <MenuItemCard key={item.id} item={item} onNotice={setNotice} />)}{items.length === 0 && <Card className="border-dashed p-8 text-center text-ilarica-muted">Nenhum item no cardápio mockado.</Card>}</div>
          <BusinessHoursPanel onSaved={() => setNotice('Horários mockados salvos para validação visual.')} />
        </section>
      </div>
    </main>
  );
