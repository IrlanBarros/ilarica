...
import { confirmSellerOrderPickup, createMyCanteenProduct, deleteMyCanteenProduct, getMyCanteen, listMyCanteenProducts, listSellerOrderHistory, listSellerOrders, updateMyCanteen, updateMyCanteenProduct, updateSellerOrderStatus } from '../services';
...
export const useSellerStore = create<SellerState>((set, get) => ({
  ...
  orders: [], orderHistory: [], orderStage: 'new', isOrdersLoading: false, ordersError: null, transitioningOrderId: null, confirmingOrderId: null,
  ...
  loadOrders: async () => { set({ isOrdersLoading: true, ordersError: null }); try { const [orders, orderHistory] = await Promise.all([listSellerOrders(), listSellerOrderHistory()]); set({ orders, orderHistory, isOrdersLoading: false }); } catch { set({ isOrdersLoading: false, ordersError: 'Não foi possível carregar os pedidos da cantina.' }); } },
  ...
  resetOrders: () => set({ orders: [], orderHistory: [], orderStage: 'new', isOrdersLoading: false, ordersError: null, transitioningOrderId: null, confirmingOrderId: null }),
}));
