...
interface SellerState {
  ...
  loadOrders: (showLoading?: boolean) => Promise<void>; advanceOrder: (id: string) => Promise<void>;
...
export const useSellerStore = create<SellerState>((set, get) => ({
  ...
  loadOrders: async (showLoading = true) => {
    if (get().isOrdersLoading || get().isOrdersRefreshing) return;
    set(showLoading ? { isOrdersLoading: true, ordersError: null } : { isOrdersRefreshing: true });
    try {
      const [orders, orderHistory] = await Promise.all([listSellerOrders(), listSellerOrderHistory()]);
      set({ orders, orderHistory, isOrdersLoading: false, isOrdersRefreshing: false, ordersError: null });
    } catch {
